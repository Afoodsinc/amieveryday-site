...
- Shopper-first flow: product-led Home → searchable 29-product first range → product/recipe → honest availability roadmap. The broader category plan is clearly labeled as future ambition, not current catalogue truth.
...
- Home and Products use a centered 3×2 composition built from six exact current packshots. The stale 27-pack lineup is no longer used in-page, and every one of the 29 catalogue cards has an accessible quick view with the pack size in its accessible name.
